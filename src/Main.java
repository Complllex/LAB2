...
class MedievalBattleGame {
    ...
    public static void main(String[] args) {
        WelcomeText wText = new WelcomeText();
        wText.print();
        Fine fine = new Fine();
        ...
        int balance = 100;
        while (balance >= 10) {
            System.out.println("Ваш баланс: " + balance + " золота. Выберите войска для покупки:");
                        ...
                        if (isPositionFree(randomX, randomY, playerUnits, botUnits)) {
                            switch (choice) {
                                case 1:
                                    ...
                                    System.out.println("Мечник вступает в наши ряды");
                                    break;
                                case 2:
                                    playerUnits.add(new Spearman(randomX, randomY));
                                    System.out.println("Копьеносец вступает в наши ряды");
                                    break;
                                case 3:
                                    playerUnits.add(new Axeman(randomX, randomY));
                                    System.out.println("Топорщики вступает в наши ряды");
                                    break;
                                case 4:
                                    playerUnits.add(new Longbowman(randomX, randomY));
                                    System.out.println("Лучники с длинным луком вступает в наши ряды");
                                    break;
                                case 5:
                                    playerUnits.add(new Shortbowman(randomX, randomY));
                                    System.out.println("Лучники с коротким луком вступает в наши ряды");
                                    break;
                                case 6:
                                    playerUnits.add(new Crossbowman(randomX, randomY));
                                    System.out.println("Арбалетчики вступает в наши ряды");
                                    break;
                                case 7:
                                    playerUnits.add(new Knight(randomX, randomY));
                                    System.out.println("Рыцарь вступает в наши ряды");
                                    break;
                                case 8:
                                    playerUnits.add(new Hussar(randomX, randomY));
                                    System.out.println("Кирасир вступает в наши ряды");
                                    break;
                                case 9:
                                    playerUnits.add(new HorseArcher(randomX, randomY));
                                    System.out.println("Конный лучник вступает в наши ряды");
                                    break;
                            }
                            placed = true;
                        }
            ...
            for (Player unit : playerUnits) {
                ...
                if (action == 1) {
                    ...
                    while (MovM != true) {
                        ...
                        if(unit.checkMove(newX, newY, FIELD_SIZE, Fine(newX, newY,unit)) && isPositionFree(newX, newY, playerUnits, botUnits)) {
                            MovM = true;
                            unit.move(newX, newY);
                        }else {
                            //System.out.println("Юнит не может переместиться на столь дальнее расстояние либо мешает преграда, приобретите премиум подписку, чтобы обойти это :D");
                        }
                    }
                } else {
    ...
    private static Player randomBotUnit(int x, int y) {
        ...
        switch (choice) {
            ...
            default:
                // В случае ошибки возвращаем мечника (как запасной вариант)
                return new Swordsman(x, y);
        }
    }
    ...
    private static void createBotUnits(List<Player> botUnits, List<Player> playerUnits) {
        ...
        while (botUnits.size() < playerUnits.size()) { // Желаемое количество юнитов бота
            int randomX = FIELD_SIZE-1-random.nextInt(2); // Случайные координаты X (0 или 1)
    ...
    private static boolean isPositionFree(int x, int y, List<Player> playerUnits, List<Player> botUnits) {
        if(!IsOutOfField(x,y)) return false;
        // Проверяем, свободна ли клетка от игроков
        for (Player player : playerUnits) {
            if (player.getX() == x && player.getY() == y) {
                return false;
            }
        }
        // Проверяем, свободна ли клетка от ботов
        for (Player bot : botUnits) {
            if (bot.getX() == x && bot.getY() == y) {
                return false;
            }
        }
        // Проверяем, является ли клетка препятствием (болотом, деревом и т.д.)
        if (battlefield[x][y] == SWAMP_TILE || battlefield[x][y] == TREE_TILE) {
            System.out.println("Мешает препятствие");
            return false;
        }
        // Клетка свободна
        return true;
